...
#define VT_TEST_ARGUMENT_HPP "3.1.0"
...
namespace argument_tests::inline v3_1_0 {
...
static constexpr auto test_init = [] () -> void {
...
static constexpr auto test_to_string = [] () -> void {
  auto arg = argument(kSv1, kSv2);
  assert_equal(arg.kind(), arg_kind::flag);
  assert_true(arg.to_string() == "[--"s + string(kSv1) + "]"s);
};

static constexpr auto test_in = [] () -> void {
  auto arg = argument(kSv1, kSv2);
...
static constexpr auto test_operator_eq = [] () -> void {
  auto arg = argument(kSv1, kSv2);
  arg = ::argumentative::string(kSv3);
  assert_equal(kSv3, arg.value());
};

static constexpr auto test_is_eq_sv = [] () -> void {
  auto arg = argument(kSv1, kSv2);
...
static constexpr auto test_is_eq = [] () -> void {
  auto arg = argument(kSv1, kSv2);
  auto arh = argument(kSv1, kSv3);
